...
function LoginContent() {
    ...
    return (
        <div className="flex-1 flex flex-col min-h-screen bg-slate-50 relative">
            ...
            <div className="absolute bottom-[-10%] left-[20%] w-[60%] h-[40%] bg-blue-200/30 blob rounded-full mix-blend-multiply filter blur-3xl animate-float" style={{ animationDelay: "4s" }} />
            ...
            <div className="px-6 pt-10 pb-4 shrink-0 relative z-10">
                <div className="flex items-center justify-center gap-2">
                    <div className="w-10 h-10 rounded-xl gradient-blue flex items-center justify-center shadow-blue">
            ...
            <div className="flex-1 flex flex-col justify-center px-6 pb-12 relative z-10 max-w-md mx-auto w-full">
                <div className="mb-8 text-center">
                    <div className="inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-full bg-white/80 border border-blue-100 shadow-sm mb-6 text-[11px] font-black uppercase tracking-widest text-blue-600 backdrop-blur-md">
                ...
                <form onSubmit={handleFinalSubmit} className="space-y-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <input required type="text" value={name} onChange={e => setName(e.target.value)} placeholder="John Doe" className="w-full bg-white/50 border border-blue-50 focus:border-blue-400 focus:bg-white focus:ring-4 focus:ring-blue-100 rounded-2xl h-14 px-4 text-slate-800 placeholder-slate-400 outline-none transition-all font-bold shadow-inner" />
                    </div>

                    {/* 2. Roll Number */}
                    <div className="space-y-2">
                        <label className="text-[11px] font-bold uppercase tracking-widest text-slate-500 pl-1">Roll Number</label>
                        <input required type="text" value={roll} onChange={e => setRoll(e.target.value.toUpperCase())} placeholder="21B81A..." className="w-full bg-white/50 border border-blue-50 focus:border-blue-400 focus:bg-white focus:ring-4 focus:ring-blue-100 rounded-2xl h-14 px-4 text-slate-800 placeholder-slate-400 outline-none font-bold transition-all shadow-inner uppercase" />
                    </div>
                    ...
                    <div className="space-y-2 relative">
                        ...
                        <div className="relative">
                            <input
                                ...
                                className="w-full bg-white/50 border border-blue-50 focus:border-blue-400 focus:bg-white focus:ring-4 focus:ring-blue-100 rounded-2xl h-14 px-4 text-sm font-bold text-slate-800 outline-none transition-all shadow-inner placeholder-slate-400"
                            />
                            {showDeptDropdown && (
                                <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-100 rounded-2xl shadow-xl shadow-slate-200/50 max-h-56 overflow-y-auto z-50 p-2 flex flex-col gap-1">
                                    ...
                                    {department && !DEPARTMENTS.some(d => d.toLowerCase() === department.toLowerCase()) && (
                                        <button
                                            ...
                                            className="text-left w-full px-3 py-2.5 rounded-xl bg-blue-50/50 hover:bg-blue-50 text-sm font-bold text-blue-700 transition-colors"
                                        >
                                            Use "{department}"
                                        </button>
                                    )}
                    ...
                    <div className="space-y-3 pt-4 border-t border-slate-100 border-dashed">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                            <div className="flex items-center gap-3 bg-white/50 border border-blue-50 focus-within:border-blue-400 focus-within:bg-white focus-within:ring-4 focus-within:ring-blue-100 rounded-2xl h-14 px-4 transition-all shadow-inner flex-1">
                                <span className="text-sm font-black text-blue-400 border-r border-blue-100 pr-3">+91</span>
                            ...
                            <button type="button" onClick={handleSendOtp} disabled={loading || phone.length < 10} className="h-14 px-6 rounded-2xl bg-blue-50 text-blue-600 hover:bg-blue-100 font-bold text-sm transition-all whitespace-nowrap active:scale-95 disabled:opacity-50">
                                Send OTP
                            </button>
                        </div>
                        ...
                        {confirmationResult || phone === "9876543210" || phone === "0123456789" ? (
                            <div className="animate-in fade-in slide-in-from-top-2 duration-300 pt-2">
                                <label className="text-[11px] font-bold uppercase tracking-widest text-slate-500 pl-1 block mb-2 text-blue-500">Enter OTP</label>
                                <div className="flex justify-between gap-2 max-w-[300px]">
                                    {otp.map((digit, i) => (
                                        <input
                                            ...
                                            disabled={loading}
                                            className="w-full aspect-square rounded-xl flex-1 bg-white/50 border border-blue-100 text-slate-800 text-xl font-black text-center outline-none focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all shadow-inner"
                                        />
                                    ))}
                                </div>
                            </div>
                    ...
                    <div className="space-y-4 pt-4 border-t border-slate-100 border-dashed">
                        ...
                        {!idImage ? (
                            <div className="space-y-2">
                                <button type="button" onClick={() => document.getElementById("ocr-input")?.click()} className="w-full h-16 rounded-2xl bg-slate-50 hover:bg-white border-2 border-blue-100 border-dashed text-blue-600 font-bold text-sm flex items-center justify-center gap-3 transition-all group shadow-sm">
                                    <Camera className="w-6 h-6 text-blue-400 group-hover:text-blue-600 group-hover:scale-110 transition-transform" />
                                    Open Camera
                                </button>
                            ...
                            <div className="space-y-3">
                                <div className="relative aspect-video rounded-2xl overflow-hidden border-2 border-blue-100 bg-slate-50 shadow-sm max-w-[280px] mx-auto">
                                    ...
                                    {loading && ocrProgress > 0 && (
                                        <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-white/40 backdrop-blur-sm">
                                            ...
                                            <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest bg-white/80 px-3 py-1 rounded-full shadow-sm border border-blue-50 animate-pulse">
                    ...
                    <div className="pt-6">
                        <button
                            ...
                            className="w-full h-14 rounded-2xl gradient-blue text-white font-black text-base shadow-blue flex items-center justify-center gap-2 active:scale-[0.98] hover:-translate-y-0.5 disabled:opacity-50 disabled:hover:translate-y-0 disabled:active:scale-100 transition-all"
                        >
...
export default function LoginPage() {
    return (
        <Suspense fallback={<div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center text-blue-600"><Loader2 className="w-10 h-10 animate-spin mb-4" /><p className="font-bold font-outfit text-xl">Loading Auth...</p></div>}>
            <LoginContent />
        </Suspense>
    );
}
