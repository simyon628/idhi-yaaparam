...
export default function ChatPage() {
    ...
    return (
        <div className="flex-1 flex flex-col min-h-screen bg-slate-50 relative">
            {/* Header */}
            <header className="fixed top-0 left-0 right-0 z-40 bg-white/85 backdrop-blur-2xl border-b border-blue-100 px-5 pt-11 pb-3 shadow-sm mx-auto max-w-md">
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-[10px] font-bold text-blue-500 uppercase tracking-widest">{rental?.itemName}</p>
                    </div>
                    {/* QR Code Toggle */}
                    {rental?.status === "active" && (
                        <button
                            onClick={() => setShowQR(!showQR)}
                            className={`p-2 rounded-xl border transition-all ${showQR ? "bg-blue-600 border-blue-600 text-white" : "bg-slate-50 border-slate-200 text-slate-500"}`}
                        >
            ...
            <AnimatePresence>
                {showQR && (
                    ...
                    >
                        <div className="bg-white/95 backdrop-blur-md border border-blue-100 rounded-3xl p-6 shadow-premium text-center">
            ...
            <main className="flex-1 pt-[105px] pb-[110px] px-4 flex flex-col gap-2">
                {/* Safety Notice */}
                <div className="bg-blue-50 border border-blue-100 p-3 rounded-2xl flex items-start gap-2.5 mb-2 shadow-sm">
                    ...
                    <AnimatePresence initial={false}>
                        {messages.map((msg) => {
                            ...
                            return (
                                ...
                                >
                                    {msg.imageUrl ? (
                                        <div className={`max-w-[70%] overflow-hidden rounded-2xl border-2 shadow-sm ${isMe ? "border-blue-200 rounded-tr-sm" : "border-slate-200 rounded-tl-sm"}`}>
                                            <img src={msg.imageUrl} alt="shared" className="w-full h-auto object-cover max-h-48" />
                                        </div>
                                    ) : (
                                        <div className={`max-w-[78%] px-4 py-2.5 rounded-2xl text-sm font-medium shadow-sm ${isMe ? "bg-blue-600 text-white rounded-tr-sm" : "bg-white border border-slate-200 text-slate-800 rounded-tl-sm"}`}>
                                            {msg.text}
                                        </div>
                                    )}
                                </motion.div>
            ...
            {/* Input Bar */}
            <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-2xl border-t border-blue-50 p-3 pb-safe z-40 shadow-[0_-10px_40px_-10px_rgba(110,115,200,0.15)] mx-auto max-w-md">
                {isCompleted ? (
                    ...
                    <div className="flex flex-col gap-2.5">
                        ...
                        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-0.5">
                            {QUICK_ACTIONS.map(action => (
                                <button
                                    ...
                                    className="whitespace-nowrap px-3 py-1.5 rounded-full bg-slate-100 hover:bg-blue-50 hover:text-blue-600 text-slate-600 text-[11px] font-bold transition-colors border border-slate-200 active:scale-95"
                                >
                                    {action}
                                </button>
                            ))}
                        ...
                        <div className="flex gap-2 items-center">
                            ...
                            <button
                                ...
                                className="w-11 h-11 flex-shrink-0 rounded-xl bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-400 hover:text-blue-500 hover:bg-blue-50 transition-all active:scale-90 disabled:opacity-50"
                            >
                            ...
                            <form onSubmit={sendMessage} className="flex-1 flex gap-2">
                                <input
                                    ...
                                    placeholder="Message..."
                                    className="flex-1 bg-slate-50 border border-slate-200 rounded-2xl px-4 h-11 text-sm font-semibold outline-none focus:border-blue-400 focus:bg-white focus:ring-4 focus:ring-blue-100 transition-all shadow-inner placeholder-slate-400"
                                />
                                <motion.button
                                    type="submit"
                                    disabled={!newMessage.trim()}
                                    whileTap={{ scale: 0.85 }}
                                    className="w-11 h-11 rounded-2xl gradient-blue text-white flex items-center justify-center shadow-blue disabled:opacity-50 transition-opacity"
                                >
