...
export default function MyRentalsPage() {
    ...
    if (loading) return (
        <div className="flex-1 flex flex-col items-center justify-center min-h-screen bg-slate-50 text-blue-600">
    ...
    return (
        <div className="flex-1 flex flex-col bg-slate-50 relative min-h-screen pb-32">
            ...
            <div className="fixed bottom-[-10%] left-[20%] w-[60%] h-[40%] bg-blue-200/30 blob rounded-full mix-blend-multiply filter blur-3xl animate-float pointer-events-none" style={{ animationDelay: "4s" }} />

            <div className="relative z-10 w-full max-w-md mx-auto">
                <header className="px-6 py-10 sticky top-0 bg-slate-50/80 backdrop-blur-xl z-20 border-b border-blue-50/50">
                ...
                <Tabs defaultValue="incoming" className="px-5 mt-6">
                    <TabsList className="w-full bg-white/60 backdrop-blur-md border border-blue-50 p-1.5 h-16 rounded-[2rem] mb-8 shadow-sm">
                        <TabsTrigger value="incoming" className="flex-1 rounded-[1.5rem] font-bold text-xs uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:text-blue-600 data-[state=active]:shadow-sm text-slate-500 transition-all">Incoming</TabsTrigger>
                        <TabsTrigger value="borrows" className="flex-1 rounded-[1.5rem] font-bold text-xs uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:text-blue-600 data-[state=active]:shadow-sm text-slate-500 transition-all">My Borrows</TabsTrigger>
                    </TabsList>

                    <TabsContent value="incoming" className="space-y-5 outline-none">
                        {incoming.length === 0 ? (
                            <div className="text-center py-20 bg-white/70 backdrop-blur-xl rounded-[2.5rem] border border-blue-50 shadow-sm">
                                <div className="bg-blue-50 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 border border-blue-100/50 shadow-inner">
                                    <ArrowRightLeft className="text-blue-300 w-10 h-10" />
                                </div>
                                <p className="text-slate-400 font-bold text-lg">No active requests</p>
                            </div>
                        ) : (
                            incoming.map((item) => (
                                <div key={item.id} className="bg-white/70 backdrop-blur-xl p-6 rounded-[2rem] border border-blue-50 shadow-sm flex items-center justify-between hover:shadow-md transition-all hover:-translate-y-0.5 group">
                                    <div className="flex items-center gap-5">
                                        <div className="w-14 h-14 rounded-2xl bg-blue-50 border border-blue-100 flex items-center justify-center text-3xl shadow-inner shrink-0 group-hover:scale-105 transition-transform">
                                            {item.icon}
                                        </div>
                                        <div>
                                            <h3 className="font-black text-lg text-slate-800">{item.itemName}</h3>
                                            <p className="text-[11px] font-black text-blue-500 uppercase tracking-widest mt-1">Status: {item.status}</p>
                                        </div>
                                    </div>
                                    {item.status === "requested" ? (
                                        <div className="flex gap-2">
                                            <button
                                                ...
                                                className="gradient-blue text-white p-3 rounded-2xl shadow-blue active:scale-95 hover:-translate-y-0.5 transition-all w-12 h-12 flex items-center justify-center"
                                            >
                    ...
                    <TabsContent value="borrows" className="space-y-5 outline-none">
                        {myBorrows.length === 0 ? (
                            <div className="text-center py-20 bg-white/70 backdrop-blur-xl rounded-[2.5rem] border border-blue-50 shadow-sm">
                                <div className="bg-blue-50 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 border border-blue-100/50 shadow-inner">
                                    <PackageCheck className="text-blue-300 w-10 h-10" />
                                </div>
